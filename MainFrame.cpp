        ...
        wxDefaultPosition, wxSize(1280, 720))
{
    ...
    // Création du menu d'aide avec bouton À propos
    wxMenuBar* menuBar = new wxMenuBar();
    wxMenu* helpMenu = new wxMenu();
    helpMenu->Append(wxID_ABOUT, "&À propos\tF1", "Informations sur l'application");
...
void MainFrame::OnAbout(wxCommandEvent& /*evt*/)
{
    ...
    wxDialog* aboutDlg = new wxDialog(this, wxID_ANY, "À propos",
        wxDefaultPosition, wxSize(450, 480));

    aboutDlg->SetBackgroundColour(wxColour(10, 14, 30));

    wxBoxSizer* mainSizer = new wxBoxSizer(wxVERTICAL);

    // Titre
    wxStaticText* title = new wxStaticText(aboutDlg, wxID_ANY,
        "Application de Traitement d'Images");
    wxFont titleFont = title->GetFont();
    titleFont.SetPointSize(14);
    titleFont.SetWeight(wxFONTWEIGHT_BOLD);
    title->SetFont(titleFont);
    title->SetForegroundColour(wxColour(0, 255, 190));
    mainSizer->Add(title, 0, wxALIGN_CENTER | wxALL, 15);

    // Version et formation
    wxStaticText* version = new wxStaticText(aboutDlg, wxID_ANY,
        "Version 1.0 | M1 EEA/TSI | 2025-2026");
    version->SetForegroundColour(wxColour(230, 240, 255));
    mainSizer->Add(version, 0, wxALIGN_CENTER | wxBOTTOM, 10);

    mainSizer->Add(new wxStaticLine(aboutDlg), 0, wxEXPAND | wxLEFT | wxRIGHT, 20);

    // Titre
    wxStaticText* teamTitle = new wxStaticText(aboutDlg, wxID_ANY, "Réalisé par :");
    wxFont teamFont = teamTitle->GetFont();
    teamFont.SetPointSize(12);
    teamFont.SetWeight(wxFONTWEIGHT_BOLD);
    teamTitle->SetFont(teamFont);
    teamTitle->SetForegroundColour(wxColour(0, 255, 190));
    mainSizer->Add(teamTitle, 0, wxALIGN_CENTER | wxTOP, 15);
    ...
    wxString info =
        ...
        "• BOUDAOUD Meriem\n"
        "• ARAB Adem\n"
        "• BENOUALI Ramzi Touhami\n";

    wxStaticText* infoText = new wxStaticText(aboutDlg, wxID_ANY, info);
    infoText->SetForegroundColour(wxColour(230, 240, 255));
    mainSizer->Add(infoText, 1, wxALL | wxALIGN_CENTER, 20);

    mainSizer->Add(new wxStaticLine(aboutDlg), 0, wxEXPAND | wxLEFT | wxRIGHT, 20);

    // Lien GitHub
    wxHyperlinkCtrl* githubLink = new wxHyperlinkCtrl(aboutDlg, wxID_ANY,
        "GitHub : image-treatment-software",
        "https://github.com/Siham-sdn/image-treatment-software");
    githubLink->SetNormalColour(wxColour(0, 255, 190));
    githubLink->SetVisitedColour(wxColour(0, 200, 150));
    githubLink->SetHoverColour(wxColour(0, 255, 220));
    mainSizer->Add(githubLink, 0, wxALIGN_CENTER | wxTOP | wxBOTTOM, 10);

    // Bouton OK
    wxButton* okBtn = new wxButton(aboutDlg, wxID_OK, "OK");
    okBtn->SetBackgroundColour(wxColour(20, 26, 50));
    okBtn->SetForegroundColour(wxColour(230, 240, 255));
    mainSizer->Add(okBtn, 0, wxALIGN_CENTER | wxALL, 15);

    aboutDlg->SetSizer(mainSizer);
    aboutDlg->Centre();
    aboutDlg->ShowModal();
    aboutDlg->Destroy();
}